						...
						<div key={question.correctAnswer + i} className={`transition-all duration-500 ${i === 0 ? '' : 'slide-right'} absolute text-center w-full`} id={'question-' + (i + 1)}>
							{
								question.ia && <div className='absolute -top-6 -right-6 p-2'>
									<Image src={starIcon} width={50} height={50} alt='-' title='-' />
								</div>
							}
							...
							<ul className={`md:columns-2 mt-4 ${'answers-' + (i + 1)}`}>
								{question.answers.map((answer, j) => (
									<li key={j + answer} className="relative">
										...
										<Image className='absolute pointer-events-none left-2 top-1 peer-disabled:translate-y-0 peer-hover:translate-y-[0.25em] peer-active:translate-y-[0.75em] transition-transform z-20 invert' src={`/letters/letter-${['ha','le','hameruha','me'][j]}.svg`} width={40} height={40} alt={`Question ${j + 1}]}`} />
									</li>
								))}
							</ul>
						</div>
